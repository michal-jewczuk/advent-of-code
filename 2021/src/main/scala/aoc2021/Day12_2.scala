...
object Day12_2 extends App {
  ...
  def convertToMap(connections: List[String]): Map[String, Set[String]] = {
    val tmp: mutable.Map[String, Set[String]] = mutable.Map("start" -> Set(), "big" -> Set(), "small" -> Set())
    ...
    def addToMap(elems: Array[String]): Unit = {
      ...
      if (first.equals("start")) tmp += "start" -> (tmp("start") + second)
        ...
        else tmp += "small" -> (tmp("small") + first)
        if (second(0).toInt <= 90) tmp += "big" -> (tmp("big") + second)
        else tmp += "small" -> (tmp("small") + second)
      }
    }
  ...
  def countPaths(entries: List[String]): Set[List[String]] = {
    ...
    def canBeAdded(path: List[String], element: String): Boolean = {
      !path.contains(element)
        || connections("big").contains(element)
        || !connections("small").exists(e => path.count(_.equals(e)) == 2)
    }

    def appendElement(path: List[String], element: String): List[String] = {
      if (path.head.equals("end")) path
      else if (canBeAdded(path, element)) element +: path
      else List()
    }
